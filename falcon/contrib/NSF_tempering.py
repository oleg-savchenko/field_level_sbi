...
class Network(torch.nn.Module):
    def __init__(self, theta, s, theta_norm = False):
        super(Network, self).__init__()
        self.theta_norm = LazyOnlineNorm(momentum=5e-3) if theta_norm else None
    ...
    def sample(self, num_samples, s):
        ...
        if self.theta_norm is not None:
            samples = self.norm.inverse(samples).detach()
        return samples
...
class NSFNode:
    ...
    def _aux_sample(self, num_samples, mode = None, parent_conditions=[], evidence_conditions=[]):
        ...
        print("Effective samples:", ((weights**2).sum(dim=0)**0.5).min())
